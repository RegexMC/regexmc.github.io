...
exports.run = async (discordClient, hypixelClient, message, args) => {
    ...
    if (message.attachments.size > 0) {
        ...
        findSauce(img, "sauce")
            ...
            .catch((error) => {
                message.reply(
                    "Sauce not found. Try using `>sauce trace` if it is a screenshot."
                );
            });
    } else {
        if (args.length > 0) {
                        ...
                        findSauce(img, trace ? "trace" : "sauce")
                            ...
                            .catch((error) => {
                                message.reply(
                                    ...
                                    (trace ?
                                        " " :
                                        " Try using `>sauce trace` if it is a screenshot.")
                                );
                            });
                    }
                }).catch(console.error);
...
function getEmbed(results, method, message) {
    ...
    if (method === "trace" || method === "trace.moe" || method === "moe") {
        ...
        if (res.size == 0) return message.reply("Nothing found!");

        var thumb = `https://trace.moe/thumbnail.php?anilist_id=${res[0].anilist_id}&file=${encodeURIComponent(res[0].filename)}&t=${res[0].at}&token=${res[0].tokenthumb}`;
        ...
        if (results.size == 0)
            return message.reply(
                "Nothing found! Try using `>sauce trace` if it's a screenshot"
            );
